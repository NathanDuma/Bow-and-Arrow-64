...
static int render(map *self) {
    ...
    if (self->mapNumber % 2 == 0) {
        ...
        // crash if list cannot be traversed
        //debug
        //int *crash = NULL;

        if (!testtraversalllist(&self->e)) {
            printDebugMessage("self->e cannot be traversed");
            //self->enemyCount = *crash;
        }
        if (!testtraversalllist(&self->h->w)) {
            printDebugMessage("self->h->w cannot be traversed");
            //self->enemyCount = *crash;
        }

        // check collison between enemies and weapon
        for (llist *itWeapon = self->h->w; itWeapon; itWeapon = itWeapon->next) {
            for (llist *itEnemy = self->e; itEnemy; itEnemy = itEnemy->next) {
                if (!((enemy*) itEnemy->node)->hit && !((enemy*) itEnemy->node)->offScreen) {
                    ((enemy*) itEnemy->node)->checkCollison(itEnemy->node, itWeapon->node, self->h);
                }
            }
        }
        ...
        if (self->h->w == NULL) printDebugMessage("self->h->w is NULL");
        for (llist *itWeapon = self->h->w; itWeapon != NULL; itWeapon = itWeapon->next) {

            ((weapon*) itWeapon->node)->playNextAnimation(itWeapon->node, self->disp);
            printDebugMessage("Weapon");

            // remove the weapon from llist if it's off screen
            if (((weapon*) itWeapon->node)->offScreen && ((weapon*) itWeapon->node)->shot) {
                printDebugMessage("OFF SCREEN");
                // destruct the weapon then delete the element in the linked list
                ((weapon*) itWeapon->node)->destructWeapon(itWeapon->node);
                deleteitemllist(&itWeapon);
        ...
        if (self->e == NULL) printDebugMessage("self->e is NULL");
        // draw all enemies on the screen
        for (llist *itEnemy = self->e; itEnemy; itEnemy = itEnemy->next) {
            ((enemy*) itEnemy->node)->playNextAnimation(((enemy*) itEnemy->node), self->disp);

            // enemy is off screen, delete it
            if ((((enemy*) itEnemy->node)->v->x == -100) &&
                    (((enemy*) itEnemy->node)->v->y == -100)) {

                printDebugMessage("Deleting enemy");

                ((enemy*) itEnemy->node)->destructEnemy(itEnemy->node);
                deleteitemllist(&itEnemy);
        ...
        if (self->e == NULL) { // all enemies are dead
            return self->mapNumber + 1; // progress
        } else if (self->h->hit || (self->h->quiverCount == 0 &&
